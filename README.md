...
The GPU Frustum Culling working perfectly fine, but there is a huge drawback which is high memory usage. A game with a similar grass rendering technique, Ghost of Tsushima, uses a sub divivde chunk system (basically, divide big field into various tiny fields and use these tiny field's AABB to cull) which is much less memory-intensive and computationally intensive.
